...
public class BrowserInteractions {
	...
	public static Logger logger = LogManager.getLogger(BrowserInteractions.class.getName());
	static WebElement element;
	static WebElement element2;
	static Screen screen = new Screen();
	static String filepath = "C:\\VPs\\Browsers\\";
	...
	public static void selectValueFromDropDown(String selector, DropdownSelectMethod option, String optionValue,
			LocatorType type) throws Exception {
	...
	public static Properties propertyFileRead() {
		...
		String location = currentDir + "/src/test/resources/";
		try {
			input = new FileInputStream(location + BaseTestSuite.propertyFileName);
	...
	public static WebElement getElement(LocatorType type, String selector, boolean checkForPresence) throws Exception {
		...
		try {
			...
			throw new Exception(
					"Element not visible in the page" + "with locator: " + selector + " Type used:" + type.name());
		}
		return element;
	}
	...
	public static void waitUntilElementIsHidden(String selector, LocatorType type, boolean checkForPresence) {
		...
		try {
			...
			FluentWait<WebDriver> wait = new FluentWait<WebDriver>(browser).withTimeout(45, TimeUnit.SECONDS)
					.pollingEvery(2, TimeUnit.MILLISECONDS).ignoring(NoSuchElementException.class);
	...
	/**********************************************************************
	 * Sikuli Common methods
	 * 
	 * @throws IOException
	 */

	public static void clickGUI(String selector) throws FindFailed, IOException {
		try {
			screen.click(filepath + browserName + "\\" + selector);
		} catch (FindFailed e) {
			SeleniumUtils.takeSnapshot();
		}
	...
	public static void typeGUI(String value) throws Exception {
		try {
			screen.type(value);
		} catch (Exception e) {
			SeleniumUtils.takeSnapshot();
	...
	public static void existGUI(String selector, int timeout) throws Exception {
		screen.exists(filepath + browserName + "\\" + selector, timeout);
	}

	public static void waitGUI(String selector, int timeout) throws Exception {
		screen.wait(selector, timeout);
	}

	public static void pasteGUI(String value) throws Exception {
		screen.paste(value);
	}
